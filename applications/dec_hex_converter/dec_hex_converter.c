...
void dec_hex_converter_logic_move_cursor_lr(DecHexConverterState* const dec_hex_converter_state, int8_t d) {
	dec_hex_converter_state->cursor += d;

	if (dec_hex_converter_state->cursor > DEC_HEX_CONVERTER_KEYS-1) dec_hex_converter_state->cursor = 0;
	else if (dec_hex_converter_state->cursor < 0) dec_hex_converter_state->cursor = DEC_HEX_CONVERTER_KEYS-1;
	...
	if (dec_hex_converter_state->mode == ModeDec) {
		if (dec_hex_converter_state->cursor == 10) dec_hex_converter_state->cursor = 16;
		else if (dec_hex_converter_state->cursor == 15) dec_hex_converter_state->cursor = 9;
	}
}
...
void dec_hex_converter_logic_move_cursor_ud(DecHexConverterState* const dec_hex_converter_state) {
	...
	if (dec_hex_converter_state->cursor < 9) {
		...
		if (dec_hex_converter_state->mode == ModeDec && (dec_hex_converter_state->cursor >= 10 && dec_hex_converter_state->cursor <= 15)) {
			if (dec_hex_converter_state->cursor <= 12) dec_hex_converter_state->cursor = 9;
			else dec_hex_converter_state->cursor = 16;
		}
	} else {
...
// fetch number from current mode and modifies the destination one
...
static void dec_hex_converter_logic_del_number(DecHexConverterState* const dec_hex_converter_state) {
...
static void dec_hex_converter_logic_add_number(DecHexConverterState* const dec_hex_converter_state, int8_t number) {
	...
	if (
		(number > 9 && dec_hex_converter_state->mode == ModeDec) ||
		dec_hex_converter_state->digit_pos >= DEC_HEX_CONVERTER_NUMBER_DIGITS
	) return;
...
static void dec_hex_converter_render_callback(Canvas* const canvas, void* ctx) {
	...
	for (int i = 0; i < DEC_HEX_CONVERTER_KEYS; i++) {
		...
		else if (i == 16) g = DEC_HEX_CONVERTER_CHAR_DEL; // '<'
		else g = DEC_HEX_CONVERTER_CHAR_SWAP; // 's'
		...
		if (dec_hex_converter_state->cursor == i) {
			canvas_draw_box(
				canvas, _x - DEC_HEX_CONVERTER_KEY_FRAME_MARGIN,
...
static void dec_hex_converter_input_callback(InputEvent* input_event, FuriMessageQueue* event_queue) {
...
int32_t dec_hex_converter_app(void* p) {
	...
	for (bool processing = true; processing;) {
		...
		DecHexConverterState* dec_hex_converter_state = (DecHexConverterState*)acquire_mutex_block(&state_mutex);
					...
					switch(event.input.key) {
						...
						case InputKeyOk:
							if (dec_hex_converter_state->cursor < DEC_HEX_CONVERTER_KEY_DEL) {
								...
								dec_hex_converter_logic_add_number(dec_hex_converter_state, dec_hex_converter_state->cursor);
